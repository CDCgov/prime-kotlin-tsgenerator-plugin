...
open class TypeScriptGenerator(
    ...
    private val visitedClassSimpleNames: MutableSet<String> = mutableSetOf()
                ...
                if (classifier is KClass<*>) {
                    if (classifier.isSubclassOf(Iterable::class) ||
                        ...
                        if ((rawKeyType.classifier as? KClass<*>)?.java?.isEnum == true) {
                            "{ [key in ${keyType.formatWithoutParenthesis()}]: ${valueType.formatWithoutParenthesis()} }" // ktlint-disable
                        } else {
